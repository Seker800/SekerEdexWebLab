...
VFX-JS 的真实 JPEG 编码损坏效果。五个确定阶段从选定的 `quality: 0.17`、`seed: 0.35`、
`iterations: 10`、`resolutionScale: 0.63` 逐步降低损坏并恢复到高清画面，且首个损坏帧产出前不暴露
原图或效果 canvas。浏览器拒绝某个损坏 JPEG 时，适配器使用有界、确定性的 seed 重试；首帧无法
产出或 WebGL 不可用时降级到原有 10×6 裁片显现，中间阶段重试耗尽则保留最近有效帧并继续向更清晰
阶段推进。逐帧调用仍由统一调度器驱动，切图、关闭或销毁
会释放 VFX 实例并使旧异步结果失效，减少动态效果时直接进入就绪状态。冷启动恢复图片 hash 时只先
恢复内容与文件系统状态，媒体查看器会在启动门完成后再打开并开始同一显现状态机，避免动画在启动
遮罩后方提前消耗。
