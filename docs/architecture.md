...
移动端使用独立终端降级模式：窄于 720px 的视口，以及短边不超过 720px 且使用粗指针的横屏
设备，都保留终端、会话、声音与重启控制，隐藏桌面遥测和屏幕键盘。该模式跟随动态视口与安全区，
依赖系统软键盘输入，并以不触发 iOS 聚焦缩放的字号、至少 44px 的触控目标和无横向裁切作为
可执行验收条件。桌面固定画布规则不适用于此降级模式。
