...
弹层状态。图片显现由独立的 `PixelateRevealRenderer` 适配器和一次性、可取消状态机管理：真实图片
解码完成后，查看器按 `object-fit: contain` 计算实际媒体边界，适配器只在该边界内把同源图片交给
VFX-JS 的 `pixelateTransition` 预设。像素块在约 1 秒内由粗变细，最后交接给清晰原图；首个特效帧
产出前不暴露原图或效果 canvas。WebGL 不可用时降级到原有 10×6 裁片显现。
逐帧调用由统一调度器驱动，切图、关闭或销毁会释放 VFX 实例。冷启动恢复图片 hash 时只先
恢复内容与文件系统状态，媒体查看器会在启动门完成且完整桌面实际绘制一帧后再打开并开始同一显现
状态机，避免动画在启动遮罩后方提前消耗，也避免弹层与桌面完成态在同一帧直接交接。
媒体查看器通过窄的 `ImageViewerEvents` 端口报告首次打开、前后切图、缩放与选区变化；主应用负责把
这些语义事件映射到现有音效总线和历史状态。查看器不依赖 `AudioDeck`，直链、文件系统、文章内图片、
鼠标按钮和方向键因此共享同一反馈策略，程序化恢复也不会重复播放打开音效。
VFX-JS 使用视口坐标绘制图像；当桌面画布在非 16:9 视口居中留边时，渲染适配器把特效 canvas 的
原点校正回视口原点，避免留边偏移叠加到图片效果上。
