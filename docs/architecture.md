...
VFX-JS 的真实 JPEG 编码损坏效果。每次打开都会重新生成十个有界随机脉冲：从选定的
`quality: 0.17`、`iterations: 10`、`resolutionScale: 0.63` 起步，随机化每段 seed、90–210 ms 节拍、
codec 速度与少量横/纵撕裂方向，同时保持质量、损坏次数与处理分辨率整体单调趋向清晰；最后用固定
