...
| Desktop geometry | `src/assets/css/main.css`, `main_shell.css`, module CSS | `apps/clone/src/styles.css` uses a 16:9 size container and container-relative units; `scripts/verify-app.ts` gates the 1920×1080 logical canvas, 1440×810 stage inside 1440×900, and the canonical regions. The 1934×1094 source PNG container retains its Electron transparency compatibility path without changing product viewport behavior |
...
| Terminal and neofetch scene | terminal class, `main_shell.css`, canonical capture | `terminal-model.ts` owns deterministic command output, while `terminal-session.ts` owns the five independent browser sessions, history, completion, current directory, typed feedback outcomes and the filesystem-command boundary; terminal markup and styles preserve the captured shell output, both Powerline prompt segments, xterm-equivalent status/footer metrics, the exact palette raster and output baseline |
...
| Filesystem | `filesystem.class.js`, `filesystem.css`, `main.css` title/scrollbar rules, `file-icons.json`; v2.2.8 media modal behavior | `filesystem-model.ts` retains the canonical screenshot snapshot; `browser-filesystem.ts` provides the bounded navigable tree consumed by `terminal-session.ts` and adds a runtime-only typed blog tree; directory, document, image, theme and keyboard-layout activation preserve distinct intents; Markdown opens in the central reader and image files use the isolated draggable media adapter, while static capture continues to use the exact canonical entry set |
| Keyboard | `keyboard.class.js`, `keyboard.css`, `src/assets/kb_layouts/en-US.json` | frozen `public/keyboard/en-US.json`, typed `keyboard-layout.ts` adapter, keyboard markup and `keyboard-feedback.ts`; all 65 source keys, control sequences and modifier commands, original arrow SVG paths, physical keydown/keyup illumination, source release blink, empty-input focus escape, two-piece Enter overlap, and independently verified special-key optical placement for current Chromium |
| Runtime scheduling | renderer module lifecycles and globe tick | `runtime-scheduler.ts` owns telemetry intervals, the clock and the live globe frame loop; `visibilitychange` pauses all registered continuous work and resumes one copy of each task |
...
- `npm run app:verify:webkit` checks core interactions, fixed-canvas geometry and mobile terminal behavior in the second target browser engine.
