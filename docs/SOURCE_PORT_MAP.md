...
| Desktop geometry | `src/assets/css/main.css`, `main_shell.css`, module CSS | `apps/clone/src/styles.css` and canonical region bounds in `scripts/verify-app.ts` |
...
| Network globe | `locationGlobe.class.js`, `encom-globe.js`, `grid.json`, `mod_globe.css` | `edex-assets.ts`, original public assets, canonical camera, seed, four independently calibrated peer pins and six-satellite runtime snapshot, plus source-effective heading, status-value, coordinate typography and cold-start canvas placement; diagnostic layer isolation uses the same ENCOM canvas |
...
| Keyboard | `keyboard.class.js`, `keyboard.css`, keyboard layout JSON | keyboard markup and input adapter; original arrow SVG paths, source modifier child selectors, source two-piece Enter overlap, independently verified fourth/fifth-row placement and per-key special-key optical placement for current Chromium |
