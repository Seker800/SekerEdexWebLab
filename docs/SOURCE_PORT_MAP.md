...
| Keyboard | `keyboard.class.js`, `keyboard.css`, keyboard layout JSON | keyboard markup, input adapter and `keyboard-feedback.ts`; original arrow SVG paths, physical keydown/keyup illumination, source release blink, modifier mapping, two-piece Enter overlap, independently verified fourth/fifth-row placement and per-key special-key optical placement for current Chromium |
