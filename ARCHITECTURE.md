...
Runtime blog documents and media come from exactly one declared source. The repository `examples/blog` root identifies itself as `sample`; an author-selected `SEKER_CONTENT_ROOT` must live outside the public repository and identify itself as `author`. Local embedded selection is exclusive, and an empty or failed author source never falls back to samples. Production site builds contain only a runtime content adapter: `npm run deploy:site` removes local author selectors and cannot write the `content/` namespace. `npm run publish:content` independently validates the author root, creates an immutable `content/releases/<digest>` manifest plus media set, then switches the no-store `content/current.json` pointer last. Site and content versions are tracked separately by Git revision and content digest. YAML parsing, schema validation, path-collision checks and local-reference validation happen before content publication; the browser validates the public pointer, author identity, digest binding, manifest shape and release-scoped media URLs before creating the content tree. Discovery rejects unknown files, symbolic links and non-regular filesystem entries; Vite 7 create, update and delete events invalidate local embedded previews. A typed, deeply immutable content tree preserves the source hierarchy and is mounted separately from the canonical eDEX snapshot. The virtual filesystem only projects this tree into the sandbox and maps typed document and media nodes to project-owned `markdown` and `image` icon semantics; the presentation layer never guesses content type from a filename extension. Filesystem clicks, terminal navigation, relative Markdown links and static-hosting-safe hash locations converge on one controller state and history policy; non-content directories are carried in typed browser history state, while article-local anchors are resolved inside the reader without competing for the global hash route. Markdown bodies remain synchronous after the versioned manifest loads so the reader and terminal `cat` command observe the same file. Documents and media mount as interchangeable views inside one `FullscreenContentOverlay`, whose absolute bounds follow the same centered 16:9 stage as the command deck at every desktop viewport. That adapter alone owns viewport coverage, command-deck isolation, the shared close control, Escape handling and teardown; the document and media renderers own only type-specific presentation and navigation. Media sequence, zoom, dismissal and browser history stay inside the same typed content route. Media reveal effects live behind a disposable renderer adapter: VFX-JS owns the GPU shader resources without synchronous framebuffer readback, the shared runtime scheduler owns one frame loop and visibility pausing, and the viewer owns only reveal state and a tile-based fallback. The media viewer now uses the VFX-JS pixelateTransition preset for a one-second entrance, with a tile-based fallback when WebGL is unavailable.
