...
Runtime blog documents are repository Markdown files collected into a validated typed manifest during the Vite build. The virtual filesystem receives that manifest as input and only projects it into the sandbox tree. Article mode isolates the hidden terminal runtime from accessibility navigation, while modal media uses a reusable focus boundary that owns background inertness, focus containment and state restoration.
